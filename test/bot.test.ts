...
/** Slash commands take no options: each one opens a panel or a form. */
...
test('Discord link → settings → watch → find persists input and returns an explained recommendation', async () => {
  ...
  const panel = interaction('find'); await bot.handle(panel.i);
...
test('Discord inventory returns the grid image, the lock command is gone and delete removes the profile', async () => {
  ...
  const lock = interaction('lock'); await bot.handle(lock.i); assert.match(JSON.stringify(lock.replies[0]), /Unknown command/);
