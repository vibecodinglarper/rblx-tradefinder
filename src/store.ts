...
export class Store {
  ...
  constructor(path: string, private archive: ArchivePolicy = DEFAULT_ARCHIVE) {
  ...
  link(discordId: string, robloxId: number, username: string): UserProfile {
    ...
    if (existing?.robloxId !== robloxId) { this.db.prepare('DELETE FROM alerts WHERE discord_id = ?').run(discordId); this.clearSnapshot(discordId); }
  ...
  forget(id: string): void {
    this.db.exec('BEGIN');
    try { this.db.prepare('DELETE FROM users WHERE discord_id = ?').run(id); this.db.prepare('DELETE FROM alerts WHERE discord_id = ?').run(id); this.clearSnapshot(id); this.db.exec('COMMIT'); }
    catch (error) { this.db.exec('ROLLBACK'); throw error; }
  }
