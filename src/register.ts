...
try {
  ...
  if (env.DISCORD_GUILD_ID) {
    ...
    console.log(`Registered ${commandJSON.map(c => `/${c.name}`).join(', ')} in the configured server and removed any global copy.`);
  } else {
    await rest.put(global, { body: commandJSON });
    console.log(`Registered ${commandJSON.map(c => `/${c.name}`).join(', ')} globally. If commands appear twice in a server, set DISCORD_GUILD_ID and register again, or remove the server-scoped copy.`);
  }
} catch {
